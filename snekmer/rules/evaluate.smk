    ...
    run:
        ...
        for seqid in list(seq_kmer_dict):
            ...
            if x not in master_seq_set:
                ...
                seq_kmer_dict[
                    (master_seq_annot_dict[x] + "_known_" + str(count))
        ...
        counts_and_sums_df = pd.DataFrame(
            np.insert(counts_and_sums_df.values, 0, values=kmer_totals, axis=0)
        )
        ...
        new_associations = counts_and_sums_df.iloc[1:, 1:].div(
            counts_and_sums_df["Sequence count"].tolist()[1:], axis="rows"
        )
        ...
        if method in ["score_enhanced", "score"]:
            if len(str(new_associations.columns.values[10])) == len(
                str(compare_df.columns.values[10])
            ):
                compare_check = True
                ...
                sys.exit(
                    "Please confirm kmer lengths match in config files of snekmer 'learn' and 'apply'."
                )
            ...
            if compare_check == True:
                ...
                alphabet_initial = set(
                    itertools.chain(
                        *[list(x) for x in new_associations.columns.values[10:check_1]]
                    )
                )
                alphabet_compare = set(
                    itertools.chain(
                        *[list(x) for x in compare_df.columns.values[10:check_1]]
                    )
                )
            ...
            # Cosine
        if method == "cosine":
            if len(str(counts_and_sums_df.columns.values[10])) == len(
                str(compare_df.columns.values[10])
            ):
                compare_check = True
            else:
            ...
            if compare_check == True:
                ...
                alphabet_initial = set(
                    itertools.chain(
                        *[
                            list(x)
                            for x in counts_and_sums_df.columns.values[10:check_1]
                        ]
                    )
                )
                alphabet_compare = set(
        ...
        if method == "cosine":
            ...
            counts_and_sums_df.drop(
                columns=counts_and_sums_df.columns[-1:], axis=1, inplace=True
            )
            counts_and_sums_df.drop(
                columns=counts_and_sums_df.columns[:1], axis=1, inplace=True
            )
            ...
            d = sklearn.metrics.pairwise.cosine_similarity(
                compare_df, counts_and_sums_df
            ).T
            print("long calc finished for: ", f)
            final_matrix_with_scores = pd.DataFrame(
                d, columns=compare_df.index, index=counts_and_sums_df.index
            )
            counts_and_sums_df = []
            compare_df = []
        if method == "score_enhanced":
            compare_df = compare_df.replace(0, np.nan)
            compare_df = compare_df.where(
                compare_df.notna(), -compare_df.mean(axis=1), axis=0
            )
        if method == "score" or method == "score_enhanced":
            d = new_associations.values @ compare_df.values.T
            final_matrix_with_scores = pd.DataFrame(
                d, columns=compare_df.index, index=new_associations.index
            )
        ...
        if config["learnapp"]["save_results"] == True:
            out_name = (
                "output/eval_apply/Seq-Annotation-Scores-" + str(f)[14:-4] + ".csv"
            )
        ...
        if config["learnapp"]["save_summary"] == True:
            ...
            for i, item in enumerate(order):
                w.append(
                    (
                        final_matrix_with_scores[
                            final_matrix_with_scores.columns[[item]]
                        ][i : i + 1]
                    ).values.tolist()[0]
                )
            vals = pd.DataFrame(w)
            ...
            summary = pd.concat(
                [names[col] + "," + vals[col] for col in names], axis="columns"
            )
    ...
    run:
        for j, f in enumerate(input.eval_apply_data):
            ...
            x = a[
                ...
                np.argpartition(-a, np.arange(2), axis=1)[:, :2],
            ]
            # print(x)
            ...
            true_overlap = sorted(
                list(set(possible_vals) & set(true_running_crosstab.columns))
            )
            true_non_overlap = sorted(
                list(
                    set(possible_vals)
                    ^ set(true_running_crosstab.columns.astype(float))
                )
            )
            false_overlap = sorted(
                list(set(possible_vals) & set(false_running_crosstab.columns))
            )
            false_non_overlap = sorted(
                list(
                    set(possible_vals)
            ...
            add_to_true_df = pd.DataFrame(
                0, index=add_to_true, columns=true_running_crosstab.columns
            )
            add_to_false_df = pd.DataFrame(
                0, index=add_to_false, columns=false_running_crosstab.columns
            )
            ...
            true_running_crosstab = true_running_crosstab.assign(
                **true_non_overlap_dict
            )
            ...
            false_running_crosstab = false_running_crosstab.assign(
                **false_non_overlap_dict
            )
        ...
        csv.write_csv(
            ...
            "output/eval_conf/true_total.csv",
        )
        csv.write_csv(
            pa.Table.from_pandas(false_running_crosstab),
            "output/eval_conf/false_total.csv",
        )
        csv.write_csv(
            pa.Table.from_pandas(ratio_running_crosstab),
            "output/eval_conf/ratio_total.csv",
        )
        ...
        imputed_df = pd.DataFrame(
            imputed_df,
            index=ratio_running_crosstab.index,
            columns=ratio_running_crosstab.columns,
        )
        print("Imputed DF: \n", imputed_df)
